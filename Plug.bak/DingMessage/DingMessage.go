...
/*
	先启用 GatewayDemon
		...
		// 重试次数 10
		github.com/XRSec/Go-Wechaty-BotDaemon()
...
func onHeartbeatGateway() {
	...
	if _, err = c.AddFunc("0 * * * *", func() {
		...
		if roomID = NewGlobleService().GetBot().Room().Find(viper.GetString("BOT.GROUP")); roomID == nil {
...
func onHeartbeat() {
	...
	if _, err = c.AddFunc("@hourly", func() {
		...
		var roomID _interface.IRoom
		// 得不到 roomID 所以出问题了
		if roomID = NewGlobleService().GetBot().Room().Find(&schemas.RoomQueryFilter{Id: viper.GetString("BOT.GROUP")}); roomID == nil {
