...
func onMessage(context *wechaty.Context, message *user.Message) {
	...
	if !m.Status {
		if message.Text() == "加群" || message.Text() == "交流群" {
			...
			out := viper.GetStringMap("GROUP.LISTS")
			//out := GetStringMapFixed("GROUP.LISTS")
		...
		/* TODO GetStringMap */
		if strings.Contains(fmt.Sprintf("%v", viper.GetStringMap("GROUP.LISTS")), message.Text()) {
			for i, v := range viper.GetStringMap("GROUP.LISTS") {
				if strings.Contains(message.Text(), v.(string)) && v != "" {
					//	邀请好友进群
					if err = message.GetWechaty().Room().Load(i).Add(message.Talker()); err != nil {
